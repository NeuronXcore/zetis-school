...
- **`/cloture`** — fin de session (encore lucide) : met à jour `MEMORY.md` (+ `TROUBLESHOOTING.md` / `ARCHITECTURE.md` si nécessaire), remet la carte Graphify à jour, et rend la checklist 9 points. **Ne committe pas** : l’humain vérifie (tests, diff) puis committe — ou appelle `/livraison`, qui prend la suite. ⚠️ L’étape **4bis** (remettre `MEMORY.md` au réel après le merge) appartient désormais à `/livraison`.
- **`/livraison`** — 🔴 **la seule commande qui committe et merge d’elle-même.** Elle enchaîne commit → push → PR → attente de la **CI** → merge → **4bis**, après `/cloture`. Trois garde-fous, chacun payé : le seul vert qui autorise un merge est celui de la **CI** (le 2026-08-22, 914 verts en local et la CI rouge) · un rouge **étranger** au chantier donne droit à **un** re-run, dont **aucune des deux issues ne merge** — vert, il révèle une loterie sans la réparer ; rouge à nouveau, c'est un défaut **reproductible**, et il n'y a jamais de second re-run · un **CHOIX de surface** (libellé, ordre, formulation) l’arrête toujours, quelle que soit la couleur de la CI — la **vérification** visuelle, elle, lui appartient, capture à l’appui. ⚠️ N’utilise **jamais** `gh pr merge --auto` : il annule le deuxième garde-fou.
