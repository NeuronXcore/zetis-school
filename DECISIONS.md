...
- `docs/decisions/adr-0013-generation-cartes-srs.md` — **Génération et cycle de vie des cartes de révision (SRS)** : le SRS est un client du substrat canonique (ADR-0011) — le contenu d'une carte dérive du **cours validé** de la leçon, 100 % local (ADR-0008). Déclencheur = **page Papa « Cartes SRS »**, génération explicite **par matière** (`/api/memory/cards/*`) — PAS d'auto-génération à la validation (arbitrage tranché : aucun dérivé du cours n'est un side-effect silencieux). **Upsert clé `(student, skill, card_type)` préservant la planification** (3 branches : A maj contenu / B création due-now / C notion orpheline → suspendue, planif conservée, réactivable) — **jamais de suppression** de ligne. Les cartes héritent de la validation de leur leçon (pas de file de relecture par carte) ; cas dégradé sans cours validé → `pending`, filtrée serveur — Accepté (2026-07-04)
- `docs/decisions/adr-0014-moteur-quiz-unifie.md` — **Moteur de quiz unifié (Lot 1)** : un service partagé pour les quatre contextes (`diagnostic`/`mission`/`revision`/`capsule_post_test`), **deuxième client du substrat canonique** (ADR-0011) — un quiz de fin de cours se génère depuis le **cours validé** d'une leçon, moteur **local** (ADR-0008). Sept formats à **correction déterministe** (`mcq`, `mcq_multi`, `true_false`, `cloze`, `numeric`, `ordering`, `matching` ; `open` = jugement LLM, reporté au Lot 2), **passe d'auto-vérification à l'aveugle** (question dont le modèle ne retrouve pas sa propre clé → écartée), **scoring pondéré par contexte** (`mission` = signal faible qui **n'ouvre jamais de lacune**). **Doctrine de validation** : les questions de quiz sont du contenu d'évaluation éphémère, servi sans relecture unitaire (traçabilité `ai_jobs` + auto-vérif + retrait a posteriori). Migration légère : `quizzes.lesson_id` + `quiz_questions.source`/`status`. Surfaces : Papa `/api/quizzes/*` + pilotage `/api/quiz-pilotage/*`, élève `/api/student/quiz*` (jamais la clé) ; XP = base d'effort + bonus score — Accepté (2026-07-05)
