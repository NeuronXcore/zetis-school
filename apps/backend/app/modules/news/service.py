...
cinq compteurs qui vivent chacun dans le module propriétaire de sa donnée. Le précédent d'un
...
#: SOURCE de ces cinq fonctions pour vérifier qu'aucune ne consomme d'échéance. La sortie, elle,
#: est un entier — elle ne peut pas trahir d'où elle vient. Aplatir ce dict en cinq appels
...
def news_summary(db: Session, student_id: int) -> dict:
    """Les cinq compteurs en un seul appel.
