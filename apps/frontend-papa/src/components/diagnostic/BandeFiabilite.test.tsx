...
describe("🔴 TROIS états, pas deux", () => {
  ...
  it("la ligne « rien à signaler » DIT SA PORTÉE — sans quoi elle ne dit rien", () => {
    // 🔴 Trouvé à la relecture visuelle du 2026-08-09 : la ligne était `text-papa-muted/70` sur une
    // bordure pointillée — presque effacée — ET muette. Or « rien vu » ne veut rien dire tant qu'on
    // ignore sur combien d'yeux ce « rien » repose. La portée compte donc PLUS ici que sur la bande
    // ambre, où d'autres faits parlent déjà.
    poser({ ...RIEN, portee: { observables: ["sortie_ecran", "copie", "taille"] } });
    expect(screen.getByText(/3/)).toBeInTheDocument();
    expect(screen.getByText(/signaux étaient observables/)).toBeInTheDocument();
    expect(screen.getByText(/« rien vu », pas un « rien eu lieu »/)).toBeInTheDocument();
  });

  it("et quand les 4 signaux étaient là, elle ne nuance pas pour rien", () => {
    poser({
      ...RIEN,
      portee: { observables: ["sortie_ecran", "copie", "taille", "plein_ecran"] },
    });
    expect(screen.queryByText(/rien eu lieu/)).not.toBeInTheDocument();
    // Elle reste PAS verte, et sans geste : les deux invariants de l'état 2.
    expect(screen.queryByRole("button")).not.toBeInTheDocument();
  });
...
describe("faits et indices ne se rendent pas pareil", () => {
  ...
  it("🔴 le contraste porte « sous le seuil » quand il n'a PAS déclenché", () => {
    // 🔴 DÉFAUT TROUVÉ À LA RELECTURE VISUELLE DU 2026-08-09, invisible à mes 36 sabotages : la
    // bande rendait 4 badges « fait » pour 3 déclencheurs. Elle se contredisait dans le geste même
    // qui explique sa règle (« un fait déclenche à lui seul »), et faisait lire un déclencheur de
    // plus qu'il n'y en avait — le motif de l'`adr-0039`, un nombre qui dit autre chose que ce
    // qu'il compte.
    //
    // ⚠️ Décor construit à la main, et c'est le point : `A_CONFIRMER` porte « contraste » dans ses
    // déclencheurs, donc il ne pouvait PAS exposer ce défaut. C'est la base de DEV qui l'a montré —
    // 3 notions sans trace sur 8, et 3×2 = 6 n'est pas > 8.
    poser({
      ...A_CONFIRMER,
      faits: { ...A_CONFIRMER.faits, acquises_sans_trace: 3, notions_total: 8 },
      declencheurs: ["sorties_ecran", "enonces_copies"], // SANS contraste
    });
    expect(screen.getAllByText("fait")).toHaveLength(2);
    expect(screen.getByText("sous le seuil")).toBeInTheDocument();
    // On ne le CACHE pas : Papa peut vouloir savoir que le contraste a frôlé.
    expect(screen.getByText(/3 notions sur 8/)).toBeInTheDocument();
    expect(screen.getByText(/il en faudrait la majorité/)).toBeInTheDocument();
  });

  it("et il redevient un FAIT dès qu'il déclenche", () => {
    // Sabotage : rendre le badge sur la présence du nombre au lieu du déclenchement → l'un des
    // deux tests rougit forcément, quel que soit le sens du sabotage.
    poser(A_CONFIRMER); // ses déclencheurs CONTIENNENT « contraste »
    expect(screen.getAllByText("fait")).toHaveLength(3);
